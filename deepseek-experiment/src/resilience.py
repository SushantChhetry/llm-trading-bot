...
class HealthChecker:
    ...
    def check_health(self, service_name: str, check_func: Callable) -> bool:
        ...
        if service_name in self.last_check and current_time - self.last_check[service_name] < self.check_interval:
...
def timeout(seconds: int):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            try:
                return asyncio.wait_for(asyncio.coroutine(func)(*args, **kwargs), timeout=seconds)
...
llm_circuit_breaker = CircuitBreaker(
    CircuitBreakerConfig(failure_threshold=3, recovery_timeout=30, expected_exception=Exception)
)
...
api_retry_handler = RetryHandler(
    RetryConfig(max_attempts=3, base_delay=1.0, max_delay=10.0, exponential_base=2.0, jitter=True)
)

database_retry_handler = RetryHandler(
    RetryConfig(max_attempts=5, base_delay=0.5, max_delay=30.0, exponential_base=1.5, jitter=True)
)
