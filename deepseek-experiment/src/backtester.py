...
class RealisticBacktester:
    ...
    def check_liquidation(self, symbol: str, current_price: float, leverage: float) -> bool:
    ...
    def _execute_trade_decision(self, timestamp: datetime, price: float, decision: Dict[str, Any]):
    ...
    def _calculate_results(self) -> BacktestResult:
        ...
        sharpe_ratio = (
            np.mean(returns) / np.std(returns) * np.sqrt(252) if len(returns) > 1 and np.std(returns) > 0 else 0.0
        )
