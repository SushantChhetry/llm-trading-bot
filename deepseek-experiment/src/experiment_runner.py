...
class ExperimentRunner:
    ...
    def run_single_experiment(
        self, experiment_params: Dict[str, Any], duration_minutes: int = 30, run_id: str = None, seed: int = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            exp_handler.setFormatter(logging.Formatter("%(asctime)s - %(name)s - %(levelname)s - %(message)s"))
            ...
            return {"experiment_id": run_id, "parameters": experiment_params, "metrics": metrics, "success": True}

        except Exception as e:
            logger.error(f"Experiment {run_id} failed: {e}")
            return {"experiment_id": run_id, "parameters": experiment_params, "error": str(e), "success": False}
        finally:
            # Restore original config
            self._restore_config(original_config)
            logger.removeHandler(exp_handler)

    def run_parameter_sweep(self, sweep_config: Dict[str, Any], duration_minutes: int = 30) -> List[Dict[str, Any]]:
        """
    ...
    def _run_experiment_bot(self, bot: TradingBot, duration_minutes: int) -> Dict[str, Any]:
        ...
        return {"trades": trades, "llm_decisions": llm_decisions}

    def _calculate_experiment_metrics(
        self, results: Dict[str, Any], start_time: datetime, end_time: datetime
    ) -> Dict[str, Any]:
