...
class ConfigManager:
    ...
    def _load_from_environment(self):
        ...
        self.trading.trading_fee_percent = float(
            os.getenv("TRADING_FEE_PERCENT", str(self.trading.trading_fee_percent))
        )
        ...
        self.trading.take_profit_percent = float(
            os.getenv("TAKE_PROFIT_PERCENT", str(self.trading.take_profit_percent))
        )
        self.trading.max_active_positions = int(
            os.getenv("MAX_ACTIVE_POSITIONS", str(self.trading.max_active_positions))
        )
        self.trading.min_confidence_threshold = float(
            os.getenv("MIN_CONFIDENCE_THRESHOLD", str(self.trading.min_confidence_threshold))
        )
        self.trading.fee_impact_warning_threshold = float(
            os.getenv("FEE_IMPACT_WARNING_THRESHOLD", str(self.trading.fee_impact_warning_threshold))
        )
        self.trading.run_interval_seconds = int(
            os.getenv("RUN_INTERVAL_SECONDS", str(self.trading.run_interval_seconds))
        )
        ...
        self.security.max_requests_per_minute = int(
            os.getenv("MAX_REQUESTS_PER_MINUTE", str(self.security.max_requests_per_minute))
        )
    ...
    def _validate_config(self):
        ...
        if self.llm.provider != "mock" and not self.security_manager.validate_api_key(
            self.llm.api_key, self.llm.provider
        ):
